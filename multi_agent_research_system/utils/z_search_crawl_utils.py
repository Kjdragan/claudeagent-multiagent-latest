...
def save_work_product(
    ...
    try:
        ...
        workproduct_content = [
            ...
            f"**Agent**: Enhanced Search+Crawl Tool (zPlayground1 integration)",
        ...
        for i, (content, url) in enumerate(zip(crawled_content, urls), 1):
            ...
            workproduct_content.extend([
                ...
                f"**Processing**: ✅ Cleaned with GPT-5-nano",
                "",
        ...
        workproduct_content.extend([
            ...
            f"- **Content Cleaning**: GPT-5-nano AI processing",
            f"- **Total Processing Time**: Combined search+crawl+clean in single operation",
            f"- **Performance**: Parallel processing with anti-bot detection",
            "",
...
async def search_crawl_and_clean_direct(
    ...
    try:
        ...
        if crawled_content_list:
            ...
            from agents.content_cleaner_agent import get_content_cleaner, ContentCleaningContext
            ...
            cleaning_contexts = []
            for content, url in zip(crawled_content_list, successful_urls):
                from urllib.parse import urlparse
            ...
            for i, (cleaned_result, (original_content, context)) in enumerate(zip(cleaning_results, cleaning_contexts)):
            ...
            try:
                from utils.research_data_standardizer import standardize_and_save_research_data
                import os
...
"""
            ...
            for i, (content, url) in enumerate(zip(crawled_content_list, successful_urls), 1):
