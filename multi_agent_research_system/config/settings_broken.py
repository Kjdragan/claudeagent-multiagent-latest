...
class EnhancedSearchConfig:
    ...
    max_total_urls_to_process: int = 50  # Maximum total URLs to process (prevents infinite loops)
    enable_success_based_termination: bool = True  # Use success-based termination vs fixed quota
    url_deduplication_enabled: bool = True  # Prevent duplicate URL crawling
    progressive_retry_enabled: bool = False  # Phase 2.3: Skip failed URLs per user directive
    max_retry_attempts: int = 1  # Phase 2.3: Only try once, then skip

    # Concurrent scraping batch configuration
    primary_batch_size: int = 16  # Primary batch size for concurrent scraping
    secondary_batch_size: int = 16  # Secondary batch size for concurrent scraping
    batch_size_calculation_method: str = "adaptive"  # "fixed", "adaptive", or "conservative"
    adaptive_batch_multiplier: float = 1.5  # Multiplier for adaptive batch sizing

    # Retry logic settings (Phase 2.3: Disabled per user directive)
    progressive_timeout_multiplier: float = 1.5  # Kept for compatibility but not used

    # Phase 2.5: PDF processing configuration
    pdf_processing_enabled: bool = True  # Enable specialized PDF processing
    pdf_extract_images: bool = False  # Don't extract images from PDFs (user preference)
    pdf_save_images_locally: bool = False  # Don't save PDF images locally
    pdf_batch_size: int = 4  # Process PDF pages in batches of 4
    pdf_timeout: int = 30000  # 30 seconds timeout for PDF processing
    pdf_content_min_length: int = 100  # Minimum PDF content length to consider successful
    ...
    crawl_retry_attempts: int = 1  # Phase 2.3: Reduced to 1 attempt per user directive  # Phase 2.3: Reduced to 1 attempt per user directive  # Only try once, then skip
...
class SettingsManager:
    ...
    def _load_environment_overrides(self):
        ...
        if os.getenv('TARGET_SUCCESSFUL_SCRAPES'):
            try:
                self._enhanced_search_config.target_successful_scrapes = int(os.getenv('TARGET_SUCCESSFUL_SCRAPES'))
            except ValueError:
                pass

        if os.getenv('MAX_TOTAL_URLS_TO_PROCESS'):
            try:
                self._enhanced_search_config.max_total_urls_to_process = int(os.getenv('MAX_TOTAL_URLS_TO_PROCESS'))
            except ValueError:
                pass

        if os.getenv('ENABLE_SUCCESS_BASED_TERMINATION'):
            self._enhanced_search_config.enable_success_based_termination = os.getenv('ENABLE_SUCCESS_BASED_TERMINATION').lower() in ('true', '1', 'yes')

        # Concurrent scraping batch configuration
        if os.getenv('PRIMARY_BATCH_SIZE'):
            try:
                self._enhanced_search_config.primary_batch_size = int(os.getenv('PRIMARY_BATCH_SIZE'))
            except ValueError:
                pass

        if os.getenv('SECONDARY_BATCH_SIZE'):
            try:
                self._enhanced_search_config.secondary_batch_size = int(os.getenv('SECONDARY_BATCH_SIZE'))
            except ValueError:
                pass

        if os.getenv('BATCH_SIZE_CALCULATION_METHOD'):
            method = os.getenv('BATCH_SIZE_CALCULATION_METHOD').lower()
            if method in ['fixed', 'adaptive', 'conservative']:
                self._enhanced_search_config.batch_size_calculation_method = method

        if os.getenv('ADAPTIVE_BATCH_MULTIPLIER'):
            try:
                self._enhanced_search_config.adaptive_batch_multiplier = float(os.getenv('ADAPTIVE_BATCH_MULTIPLIER'))
            except ValueError:
                pass
